...
Sending goes over HTTPS (Graph) rather than SMTP because Railway only allows
outbound SMTP on the Pro plan and this project runs on Hobby — on Hobby the
SMTP connection just hangs (~70 s) and fails with `ENETUNREACH`. Graph
has no such restriction. `getAccessToken()` caches the Client Credentials
token in memory (valid ~60–90 min) rather than re-authenticating before
every send. `sendGraphMail()` never
